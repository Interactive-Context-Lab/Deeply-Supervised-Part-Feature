...
def SE_ResNeXt(input_image, num_classes, is_training=False, group=32, data_format='channels_last', net_depth=50):
    ...
    # gap_block1 = tf.reduce_mean(inputs_features, [1, 2], name='global_avg_pool1', keepdims=False)
    ...
    # down_inputs = tf.layers.conv2d(inputs_features, 256, (1, 1), use_bias=True,
    #                             name='block2_1x1_down', strides=(1, 1),
    #                             padding='valid', data_format=data_format, activation=None,
    #                             kernel_initializer=tf.contrib.layers.xavier_initializer(),
    #                             bias_initializer=tf.zeros_initializer())
    #
    # gap_block2 = tf.reduce_mean(down_inputs, [1, 2], name='global_avg_pool2', keepdims=False)
    # print(np.shape(gap_block2))
    ######   block 3   #########
    ind = 2
    is_root = False
    need_reduce = True
    ...
    # down_inputs = tf.layers.conv2d(inputs_features, 256, (1, 1), use_bias=True,
    #                             name='block3_1x1_down', strides=(1, 1),
    #                             padding='valid', data_format=data_format, activation=None,
    #                             kernel_initializer=tf.contrib.layers.xavier_initializer(),
    #                             bias_initializer=tf.zeros_initializer())
    #
    # gap_block3 = tf.reduce_mean(down_inputs, [1, 2], name='global_avg_pool3', keepdims=False)
    ######   block 4   #########
    ind = 3
    is_root = False
    need_reduce = True
    ...
    # if data_format == 'channels_first':
    #     pooled_inputs = tf.reduce_mean(inputs_features, [2, 3], name='pool5/7x7_s1', keepdims=True)
    # else:
    #     pooled_inputs = tf.reduce_mean(inputs_features, [1, 2], name='pool5/7x7_s1', keepdims=True)
    #
    ...
    # finial_features = tf.concat([gap_block1, gap_block2, gap_block3], axis=1)
    return inputs_features#, finial_features #, logits_output, tf.nn.softmax(logits_output, name='prob')
